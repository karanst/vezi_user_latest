...
class StateProduct extends State<FeatureOrder> with TickerProviderStateMixin {
  bool _isLoading = true, _isProgress = false;
  final GlobalKey<ScaffoldState> _scaffoldKey = new GlobalKey<ScaffoldState>();
  List<FeatureOrderModel> orderList = [];
  List<FeatureOrderModel> tempList = [];
...
//List<SectionModel> cartList = [];
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: getSimpleAppBar("Feature Order", context,show: "data"),
        key: _scaffoldKey,
  ...
  Widget listItem(int index) {
    if (index < orderList.length) {
      FeatureOrderModel model = orderList[index];
      return Padding(
  ...
  void getOrder(String top) {
    ...
    apiBaseHelper.postAPICall(getFeatureOrderApi, parameter).then((getdata) {
      bool error = getdata["error"];
      String? msg = getdata["message"];
      ...
      if (!error) {
        ...
        var data = getdata["data"];
        for(var v in data){
          tempList.add(new FeatureOrderModel.fromJson(v));
        }
