...
class StatePayment extends State<Payment> with TickerProviderStateMixin {
 ...
 // final plugin = PaystackPlugin();
                                            ...
                                            ListView.builder(
                                                ...
                                                itemBuilder: (context, index) {
                                                  if (index == 1 && cod)
                                                    ...
                                                    return paymentItem(index);
                                                  else if (index == 6 && paystack)
                                                    return paymentItem(index);
                                                  else if (index == 7 && flutterwave)
                                                    return paymentItem(index);
                                                  else if (index == 8 && stripe)
                                                    return paymentItem(index);
                                                  else if (index == 9 && paytm)
                                                    return paymentItem(index);
  ...
  //             padding: const EdgeInsets.all(5.0),
  //             child: Text(
  //               DateFormat('dd').format(today.add(Duration(days: index+1))),
  //               style: TextStyle(
  //                   fontWeight: FontWeight.bold,
  //                   color: selectedDate == index
  //                       ? Theme.of(context).colorScheme.white
  ...
  //           ),
  //           Text(
  //             DateFormat('MMM').format(today.add(Duration(days: index+1))),
  //             style: TextStyle(
  //                 color: selectedDate == index
  //                     ? Theme.of(context).colorScheme.white
  //                     : Theme.of(context).colorScheme.lightBlack2),
  ...
  //     ),
  //     onTap: () {
  //       DateTime date = today.add(Duration(days: 0));
  //       if (mounted) selectedDate = 0 ;
  //       selectedTime = null;
  //       selTime = null;
  //       selDate = DateFormat('yyyy-MM-dd').format(date);
  //       timeModel.clear();
    ...
    if (_isNetworkAvail) {
      ...
      try {
        ...
        if (response.statusCode == 200) {
         ...
         //   print("getting settings data here now" +  data);
            ...
            if(ccAvenue) {
              ccAvenueMerID = payment['ccavnue_merchant_id'];
              ccAvenueAccessCode = payment['ccavnue_access_code'];
              print("cc avenueMerID ${ccAvenueMerID} and ${ccAvenueAccessCode}  and ${razorpayId}");
            }
            if (paystack) {
  ...
  Widget timeSlotItem(int index) {
    return new InkWell(
      onTap: () {
        if(timeModel[index].name.toString().toLowerCase().contains("click")){
              showDialog(context: context, builder: (context){
                 return AlertDialog(
                   title: Text("this allows customer to pay and collect from store"),
                   actions: [
  ...
  Widget paymentItem(int index) {
    return new InkWell(
      onTap: () {
        /*if(selTime!=null&&selTime!.toLowerCase().contains("click")&&paymentMethodList[index]!.toLowerCase().contains("cash")){
          setSnackbar("", context)
          return;
        }*/
        if (mounted)
